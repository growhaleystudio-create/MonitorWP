...
function Sidebar({ onLogout, isCollapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <>
      {/* Mobile Header */}
      <header className="md:hidden flex items-center justify-between px-6 py-4 bg-white border-b border-slate-100 fixed top-0 left-0 right-0 z-40">
        ...
        <button
          ...
          className="p-1.5 rounded text-slate-500 hover:bg-slate-50 transition"
        >
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-45 bg-[#f4f8f7] border-r border-slate-200/60 py-8 flex flex-col justify-between transform transition-all duration-300 md:translate-x-0 ${
          isOpen ? 'w-64 translate-x-0' : '-translate-x-full'
        } ${isCollapsed ? 'md:w-20 md:px-3' : 'md:w-64 md:px-5'} ${isOpen ? 'pt-24 md:pt-8' : ''}`}
      >
          ...
          <nav className="flex flex-col gap-2 mt-2">
            {navigation.map((item) => (
              ...
              >
                ...
                {/* Collapsed Tooltip */}
                {isCollapsed && (
                  <div className="absolute left-full ml-3 px-2 py-1 rounded bg-slate-900 text-white text-[10px] font-bold tracking-wide uppercase whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-md">
                    {item.name}
                  </div>
                )}
              </NavLink>
        ...
        <div className="border-t border-slate-200/60 pt-6 flex flex-col gap-4">
          ...
          <button
            ...
            className={`flex items-center justify-center rounded font-extrabold text-[12px] text-coral border border-coral/20 bg-coral/5 hover:bg-coral hover:text-white transition-all duration-150 shadow-sm ${
              isCollapsed ? 'w-10 h-10 mx-auto' : 'gap-2 px-4 py-2.5 w-full'
            }`}
